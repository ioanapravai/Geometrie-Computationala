...
int main()
{
    ...
    /// determinant pentru sistemul format cu ecuatiile celor doua drepte
    det=s1.geta()*s2.getb()-s1.getb()*s2.geta();
    if(det!=0) //Cazul 1: dreptele nu au aceeasi panta. Verif. intersectia segmentelor
    {
    ...
    else
    {
       ...
       else
       {
        if(s1.geta()!=0 || s2.geta()!=0 || s1.getb()!=0 || s2.getb()!=0 || s1.getc()!=0 || s2.getc()!=0)
        {
            ...
            {
                /// coliniaritate, verific cazurile de intersectare
